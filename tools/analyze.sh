...
# Use the analyzer from src directory
ANALYZER="${PROJECT_ROOT}/src/analyzer.py"
ARTIFACTS_ROOT="${PROJECT_ROOT}/artifacts"

# Run preflight check to ensure artifact structure exists (don't enforce)
"$PROJECT_ROOT/preflight.sh" --no-tmp-clean >/dev/null 2>&1
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -r|--results)
            OUTPUT_DIR="$2"
            # If not an absolute path and doesn't start with "./", put it in artifacts directory
            if [[ ! "$OUTPUT_DIR" = /* && ! "$OUTPUT_DIR" = ./* ]]; then
                # Create a timestamped run directory
                TIMESTAMP=$(date +%Y%m%d_%H%M%S)
                OUTPUT_DIR="$ARTIFACTS_ROOT/analysis/run_${TIMESTAMP}"
                mkdir -p "$OUTPUT_DIR"
                echo "Using canonical artifacts directory: $OUTPUT_DIR"
            fi
...
if [[ ! "$OPTIONS" =~ "--output" ]]; then
    # No output directory specified, use canonical path
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    OUTPUT_DIR="$ARTIFACTS_ROOT/analysis/run_${TIMESTAMP}"
    mkdir -p "$OUTPUT_DIR"
    OPTIONS="$OPTIONS --output \"$OUTPUT_DIR\""
