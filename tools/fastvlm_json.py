...
def run_fastvlm_json_analysis(image_path, model_path, prompt=None, max_retries=3, mode="describe"):
    ...
    # Get the predict.py script path from project root
...
def main():
    ...
    try:
        ...
        if result:
            # Determine output file path - use canonical artifact path if not specified
            output_path = args.output
            if not output_path:
                # Generate canonical artifact path based on image name and mode
                ...
                if not args.quiet:
                    print(f"\nUsing canonical artifact path: {output_path}")
            else:
                # If user specified path, validate it
                if not validate_artifact_path(output_path):
                    print(f"Warning: Output path {output_path} is not a canonical artifact path")
        ...
        # Determine output file path for error results
        output_path = args.output
        if not output_path:
            # Generate canonical artifact path for error results
            image_basename = os.path.basename(args.image)
            image_name = os.path.splitext(image_basename)[0]
            artifact_dir = get_canonical_artifact_path("vision", f"fastvlm_error_{args.mode}")
            output_path = os.path.join(artifact_dir, f"{image_name}_error.json")
            
            if not args.quiet:
                print(f"\nUsing canonical artifact path for error results: {output_path}")
        
        # Ensure directory exists
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        
        # Save error results with PathGuard
        with PathGuard(os.path.dirname(output_path)):
            with open(output_path, 'w') as f:
                json.dump(error_result, f, indent=2)
            if not args.quiet:
                print(f"\nError results saved to {output_path}")
