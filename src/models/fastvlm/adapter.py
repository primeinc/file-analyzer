...
class FastVLMAdapter:
    ...
    def _initialize_model(self, auto_download: bool = True) -> bool:
        ...
        if not self.model_path:
            ...
            if auto_download:
                logger.info(
                    f"Attempting to download {self.model_type} {self.model_size}..."
                )
        ...
        logger.debug(
            f"Initialized {self.model_type} {self.model_size} at {self.model_path}"
        )
        logger.debug(
            f"Using predict script: {'mlx-vlm package' if self.predict_script == 'package' else self.predict_script}"
        )
    ...
    def predict(
        ...
        if not self.initialized:
            if not self._initialize_model():
                raise RuntimeError(
                    f"Model initialization failed for {self.model_type} {self.model_size}"
                )
        ...
        if mock:
            ...
            if output_path:
                try:
                    ...
                    if ARTIFACT_DISCIPLINE:
                        with PathGuard(os.path.dirname(output_path)):
                            with open(output_path, "w") as f:
                                json.dump(
                                    mock_result, f, indent=2, cls=PathAwareJSONEncoder
                                )
                    else:
                        with open(output_path, "w") as f:
                            json.dump(
                    ...
                    raise RuntimeError(
                        f"Failed to write mock output to {output_path}: {e}"
                    )
        ...
        if output_path is None:
            ...
            if ARTIFACT_DISCIPLINE:
                artifact_dir = create_artifact_path_for_model_output(
                    self.model_type, f"{mode}_{self.model_size}"
                )
        ...
        try:
            ...
            if self.predict_script == "package":
                result = self._predict_with_package(
                    image_path, prompt, mode, timeout_seconds
                )
            else:
                result = self._predict_with_script(
                    image_path, prompt, timeout_seconds, output_path
                )
        ...
        if isinstance(result, dict):
            ...
            if output_path and self.predict_script == "package":
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to write model output to {output_path}: {e}"
                    )
                ...
                try:
                    ...
                    if ARTIFACT_DISCIPLINE:
                        ...
                        with open(output_path, "w") as f:
                            json.dump(json_result, f, indent=2)
                    logger.debug(
                        f"Successfully wrote processed output to: {output_path}"
                    )
                except Exception as e:
                    raise RuntimeError(
                        f"Failed to write processed output to {output_path}: {e}"
                    )
                ...
                try:
                    ...
                    if ARTIFACT_DISCIPLINE:
                        with PathGuard(os.path.dirname(output_path)):
                            with open(output_path, "w") as f:
                                json.dump(
                                    result_dict, f, indent=2, cls=PathAwareJSONEncoder
                                )
                    else:
                        with open(output_path, "w") as f:
                            json.dump(
                    ...
                    raise RuntimeError(
                        f"Failed to write raw output to {output_path}: {e}"
                    )

            return result_dict

    def _predict_with_package(
        self, image_path: str, prompt: str, mode: str, timeout_seconds: int
    ) -> dict[str, Any] | str:
        """
        ...
        try:
            ...
            if os.path.isdir(nested_path):
                ...
                if has_config and has_model:
                    ...
                    logger.info(
                        f"Detected double-nested model structure. Using {nested_path} instead of {model_path}"
                    )
    ...
    def _predict_with_script(
        ...
        cmd = [
            ...
            prompt,
            "--max_new_tokens",
            "256",  # Reduced to prevent repetition issues
        ]
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"JSON parsing failed, attempting to repair: {str(e)[:100]}"
                )
                ...
                raise RuntimeError(
                    f"Failed to parse model output as JSON: {e}. JSON repair also failed: {repair_error}. Raw output: {output[:200]}..."
                )

        # Validate required fields
        if not isinstance(parsed, dict):
            raise RuntimeError(
                f"Model output is not a valid JSON object. Got: {type(parsed)}"
            )

        if "description" not in parsed or "tags" not in parsed:
            available_keys = list(parsed.keys()) if isinstance(parsed, dict) else "N/A"
            raise RuntimeError(
                f"Model output missing required fields (description, tags). Available keys: {available_keys}"
            )
        ...
        if output_path:
            try:
                ...
                raise RuntimeError(
                    f"Failed to write model output to disk at {output_path}: {e}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt", help="Custom prompt (if omitted, uses default JSON prompt)"
    )
    parser.add_argument(
        "--output",
        help="Path to save the output JSON (if omitted, uses canonical artifact path)",
    )
    parser.add_argument(
        "--mode",
        default="describe",
    ...
    result = adapter.predict(
        args.image, args.prompt, args.output, args.mode, args.timeout
    )
