...
def run_fastvlm_json_analysis(
    ...
    if not os.path.exists(predict_script):
        ...
        if os.path.exists(alternate_path):
            ...
            logging.error(
                f"predict.py script not found at {predict_script} or {alternate_path}"
            )
            raise FileNotFoundError(
                f"predict.py script not found in expected locations: {predict_script} or {alternate_path}"
            )
    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                ...
                if JSONValidator.validate_json_structure(
                    json_data, expected_fields, mode
                ):
                ...
                if attempt < max_retries - 1:
                    logging.warning(
                        "Invalid JSON format. Retrying with stronger prompt..."
                    )
                    prompt = get_json_prompt(mode, retry_attempt=attempt + 1)
                    cmd = [
                        sys.executable,
                    ...
                    raise JSONParsingError(
                        text=output, metadata=error_result["metadata"]
                    )
...
def main():
    ...
    parser.add_argument(
        "--output", help="Output JSON file path (MUST be canonical or will be REJECTED)"
    )
    parser.add_argument(
        "--retries",
        type=int,
    ...
    parser.add_argument(
        "--prompt", help="Custom prompt (use with caution to ensure JSON output)"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="Timeout in seconds for the FastVLM subprocess",
    )
    ...
    output_path = (
        args.output
        if args.output
        else os.path.join(artifact_dir, f"{image_name}_result.json")
    )
    ...
    if result:
        ...
        if not args.quiet:
            ...
            if "metadata" in result:
                ...
                if (
                    "attempts" in result["metadata"]
                    and result["metadata"]["attempts"] > 1
                ):
