...
def get_canonical_artifact_path(type_name: str, context: str) -> Path:
    ...
    if type_name not in ARTIFACT_TYPES:
        raise ValueError(
            f"Invalid artifact type: {type_name}. Valid types: {', '.join(ARTIFACT_TYPES)}"
        )
...
def _create_artifact_manifest(
    artifact_dir: str, artifact_type: str, context: str
) -> None:
    """
    ...
    try:
        ...
        external_frames = [
            ...
            if not frame.filename.endswith(
                ("artifact_guard.py", "pathlib.py", "<frozen importlib", "__init__.py")
            )
        ...
        if external_frames:
            ...
            for frame in external_frames:
                if any(
                    ...
                    for keyword in ["test_", "_test", "script", "tools/"]
                ):
...
def validate_artifact_path(path: str) -> bool:
    ...
    if os.path.dirname(abs_path) == PROJECT_ROOT:
        ...
        if (
            ...
            or "_results" in base_name
            or base_name.startswith("fastvlm_test_")
            or base_name.startswith("analysis_")
        ):
            return False
    ...
    temp_path_patterns = [r"(/|^)temp(/|$)", r"(/|^)tmp(/|$)", r"(/|^)temporary(/|$)"]
        ...
        for param_name, param_value in bound_args.arguments.items():
            ...
            if isinstance(param_value, (str, Path)) and (
                ...
                or (
                    ...
                    and ("/" in param_value or "\\" in param_value)
                )
            ):
...
def main() -> None:
    ...
    if args.command == "create":
        ...
        print(
            f"Path {'IS' if valid else 'is NOT'} valid according to artifact discipline."
        )
