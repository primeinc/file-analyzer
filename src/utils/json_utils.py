...
class JSONValidator:
    ...
    def extract_json_from_text(text):
        ...
        if potential_jsons:
            ...
            for json_obj in potential_jsons:
                if isinstance(json_obj, dict) and any(
                    field in json_obj for field in expected_fields
                ):
                    return json_obj
            ...
            return max(
                potential_jsons,
                key=lambda x: len(json.dumps(x)) if isinstance(x, dict) else 0,
            )
        ...
        for match in matches:
            ...
            if ('"description"' in match and '"tags"' in match) or any(
                f'"{field}"' in match for field in ["objects", "text", "document_type"]
            ):
...
def process_model_output(
    output, mode="describe", metadata=None, attempt_count=0, retry_prompt=None
):
    """
    ...
    if json_data and JSONValidator.validate_json_structure(
    ...
    base_metadata = {"mode": mode, "json_parsing_failed": True}
