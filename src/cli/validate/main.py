...
def schema(
    ...
    if strict and validation_results["warnings"]:
        validation_results["success"] = False
        console.print("[red]✗[/red] Validation failed in strict mode due to warnings")
    ...
    try:
        ...
        if method == "pixel":
            ...
            try:
                ...
                if mismatch_count == 0:
                    console.print("[green]✓[/green] Images are pixel-perfect matches")
                    result["success"] = True
                else:
                    if strict:
                        console.print(
            ...
            console.print("Supported methods: pixel, hash, ssim")
...
def manifest(
    ...
    if not isinstance(manifest, dict):
        console.print("[red]Error:[/red] Manifest must be a JSON object")
        raise typer.Exit(code=1)
    ...
    if not isinstance(manifest["artifacts"], list):
        console.print("[red]Error:[/red] Manifest 'artifacts' must be an array")
        raise typer.Exit(code=1)
