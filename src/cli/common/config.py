...
def find_project_root(
    ...
    if fallback:
        logger.warning(
            f"No project root markers {markers} found. Using fallback paths."
        )
        ...
        for parent in current_path.parents:
            if parent.name == "src" and parent.parent:
                logger.debug(
                    f"Using 'src' parent directory as project root: {parent.parent}"
                )
        ...
        if len(current_path.parents) >= 3:
            logger.debug(
                f"Using 3 levels up as project root: {current_path.parents[2]}"
            )
...
class Config:
    ...
    def _use_new_config_system(self):
        ...
        logger.info(
            f"Configuration initialized with project root: {self.runtime['project_root']}"
        )
    ...
    def _use_legacy_config_system(self):
        ...
        if self.config_file:
            ...
            logger.warning(
                "No valid configuration file found, using environment variables and defaults only"
            )
    ...
    def get_model_path(
        self, model_type: str = DEFAULT_MODEL_TYPE, model_size: str = DEFAULT_MODEL_SIZE
    ) -> str | None:
        """
    ...
    def get_schema_path(self, schema_type: str, version: str = "v1.0") -> Path | None:
        ...
        if NEW_CONFIG_AVAILABLE:
            ...
            schema_locations = [
                ...
                schemas_dir
                / version
                / schema_type,  # Alternative: schemas/version/type/
            ...
            for schema_path in schema_locations:
                ...
                schema_files = [
                    ...
                    schema_path
                    / f"{schema_type}_{version}.json",  # Versioned: type_version.json
                    schema_path
                    / "schema"
                    / "schema.json",  # Nested: schema/schema.json
                ]
        ...
        if NEW_CONFIG_AVAILABLE:
            ...
            try:
                ...
                output_dir = os.path.join(
                    base_dir, "artifacts", artifact_type, f"{context}_{timestamp}"
                )
