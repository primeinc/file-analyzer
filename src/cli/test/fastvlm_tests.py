...
def run_test(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        model_info = context["config"].get_model_info("fastvlm", model_size)
    ...
    try:
        ...
        sample_dir = Path(context["config"].runtime["project_root"]) / "test_data" / "sample_images"
    ...
    # 5. Run the mock benchmark with standard test images
    if use_mock:
        if console:
    ...
    # 6. Run the actual benchmark script (if model exists and not using mock)
    ...
    # 7. Test with standard local test image
    if console:
    ...
    try:
        ...
        if test_image:
            ...
            test_data_dir = Path(context["config"].runtime["project_root"]) / "test_data" / "images"
            test_img_candidates = [
            ...
            if not test_img_path and images:
        ...
        if not test_img_path or not os.path.exists(test_img_path):
            ...
            # 8. Test different analysis modes with mock analyzer
            if console:
