    ...
    def analyze_image(self, path, prompt=None, mode="describe"):
        ...
        if mode == "describe":
            return {
                ...
                "metadata": {"time": 0.1, "model": "MockModel"},
            }
        elif mode == "detect":
            return {
                "objects": [
...
def test_analysis_modes(test_env: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if test_image:
            ...
            test_data_dir = (
                Path(config.runtime["project_root"]) / "test_data" / "images"
            )
            test_img_candidates = [
        ...
        if not test_img_path or not os.path.exists(test_img_path):
            ...
            try:
                ...
                for mode in modes:
                    try:
                        ...
                        output_file = os.path.join(
                            output_dir, f"{mode}_mode_result.txt"
                        )
                ...
                results["analysis_modes"] = {
                    "status": "ok"
                    if all(r.get("status") == "ok" for r in mode_results.values())
                    else "warning",
                    "message": "All modes tested successfully"
                    if all(r.get("status") == "ok" for r in mode_results.values())
                    else "Some modes failed",
                    "modes": mode_results,
                }
            except Exception as e:
        ...
        try:
            ...
            if image_files:
                ...
                benchmark_results = run_benchmark(
                    mock_analyzer, image_files, output_file
                )
        ...
        try:
            ...
            try:
                with open(benchmark_output_file, "w") as f:
                    ...
                    benchmark_main(
                        ["--output", benchmark_output_file, "--size", model_size]
                    )
...
def setup_sample_images(test_env: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        sample_dir = (
            Path(config.runtime["project_root"]) / "test_data" / "sample_images"
        )
        ...
        with open(sample_images_log, "w") as f:
            f.write(
                f"Using {len(images)} sample test images in {os.path.abspath(sample_dir)}\n"
            )
            for img in images:
                f.write(f"- {img}\n")
    ...
    return {"results": results, "errors": errors, "images": images}
...
def find_models(test_env: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        results["models"] = {
            ...
            "message": "Model found"
            if model_info.get("available")
            else "No model found (will use mock)",
    ...
    return {"results": results, "errors": errors, "use_mock": use_mock_result}
...
def run_test(context: dict[str, Any]) -> dict[str, Any]:
    ...
    with open(summary_path, "w") as f:
        ...
        f.write(
            f"Environment check: {results.get('environment', {}).get('status', 'unknown')}\n"
        )
        f.write(f"Model files: {results.get('models', {}).get('status', 'unknown')}\n")
        f.write(
            f"Sample images: {results.get('sample_images', {}).get('message', 'unknown')}\n"
        )
        f.write(
            f"Mock analyzer: {results.get('mock_analyzer', {}).get('status', 'not created')}\n"
        )
        f.write(f"Benchmark: {results.get('benchmark', {}).get('status', 'not run')}\n")
        f.write(
            f"Test image: {results.get('test_image', {}).get('status', 'unknown')}\n"
        )
        f.write(
            f"Analysis modes: {results.get('analysis_modes', {}).get('status', 'not tested')}\n"
        )
