...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Python to Bash adapter for artifact_guard.py"
    )
    ...
    env_parser = subparsers.add_parser(
        "env", help="Generate environment script for shell integration"
    )

    # Shell command generator
    shell_parser = subparsers.add_parser(
        "shell", help="Generate shell commands for bash integration"
    )
    shell_parser.add_argument(
        "func",
        choices=["mkdir_guard", "touch_guard", "cp_guard", "mv_guard", "aliases"],
        help="Shell function to generate",
    )

    # Batch command
    batch_parser = subparsers.add_parser(
        "batch", help="Run commands from stdin in batch mode"
    )
