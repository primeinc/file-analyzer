...
# Import artifact path enforcement
from src.artifact_guard import get_canonical_artifact_path, PathGuard

# Import vision analyzer module
try:
    from src.vision import VisionAnalyzer, DEFAULT_VISION_CONFIG, VISION_MODELS
    VISION_AVAILABLE = True
except ImportError:
    try:
        # For backward compatibility
        from src.vision_analyzer import VisionAnalyzer, DEFAULT_VISION_CONFIG, VISION_MODELS
        VISION_AVAILABLE = True
    except ImportError:
...
DEFAULT_CONFIG = {
    "default_output_dir": None,  # Will use canonical artifact path
...
class FileAnalyzer:
    def __init__(self, path, output_dir=None, verbose=True, include_patterns=None, exclude_patterns=None, config=None):
        ...
        # Set output directory using canonical artifact path
        if output_dir:
            self.output_dir = Path(output_dir)
        else:
            # Create a canonical artifact directory
            analysis_context = f"analyzer_{Path(path).name}"
            artifact_dir = get_canonical_artifact_path("analysis", analysis_context)
            self.output_dir = Path(artifact_dir)
    ...
    def extract_metadata(self):
        # Create a canonical output directory for this analysis
        metadata_dir = get_canonical_artifact_path("analysis", "metadata_extraction")
        
        if self.verbose:
            print(f"Extracting metadata from {self.path}...")
            print(f"Results will be saved to {metadata_dir}")
        ...
        try:
            ...
            try:
                ...
                debug_file = Path(metadata_dir) / "metadata_debug.txt"
            ...
            output_file = Path(metadata_dir) / "metadata.json"
    ...
    def find_duplicates(self):
        # Create a canonical output directory for this analysis
        duplicates_dir = get_canonical_artifact_path("analysis", "duplicates_scan")
        
        print(f"Finding duplicates in {self.path}...")
        print(f"Results will be saved to {duplicates_dir}")
        ...
        results_file = Path(duplicates_dir) / "duplicates.txt"
    ...
    def perform_ocr(self):
        # Create a canonical output directory for this analysis
        ocr_dir = get_canonical_artifact_path("analysis", "ocr_scan")
        
        if self.verbose:
            print(f"Performing OCR on images in {self.path}...")
            print(f"Results will be saved to {ocr_dir}")
        ...
        # Save results
        output_file = Path(ocr_dir) / "ocr_results.json"
        with open(output_file, 'w') as f:
    ...
    def scan_malware(self):
        # Create a canonical output directory for this analysis
        malware_dir = get_canonical_artifact_path("analysis", "malware_scan")
        
        print(f"Scanning for malware in {self.path}...")
        print(f"Results will be saved to {malware_dir}")
        
        command = ["clamscan", "-r", "--bell", "-i", str(self.path)]
        output = self.run_command(command)
        
        if output:
            output_file = Path(malware_dir) / "malware_scan.txt"
            with open(output_file, 'w') as f:
                f.write(output)
    ...
    def search_content(self, search_text):
        # Create a canonical output directory for this analysis
        search_dir = get_canonical_artifact_path("analysis", f"search_{search_text}")
        ...
        if output:
            output_file = Path(search_dir) / "search_results.txt"
            with open(output_file, 'w') as f:
                f.write(output)
    ...
    def analyze_binary(self):
        # Create a canonical output directory for this analysis
        binary_dir = get_canonical_artifact_path("analysis", "binary_analysis")
        
        print(f"Analyzing binary content in {self.path}...")
        print(f"Results will be saved to {binary_dir}")
        ...
        if output:
            output_file = Path(binary_dir) / "binary_analysis.txt"
            with open(output_file, 'w') as f:
                f.write(output)
    ...
    def analyze_images_with_vision(self, mode="describe"):
        ...
        # Create a canonical output directory for this analysis
        vision_dir = get_canonical_artifact_path("vision", f"vision_analysis_{mode}")
        ...
        if self.verbose:
            ...
            print(f"Results will be saved to {vision_dir}")
        ...
        if self.path.is_dir() and vision_config.get("batch_processing", False):
            ...
            batch_output_dir = Path(vision_dir) / "batch_results"
        ...
        if results:
            ...
            if output_format == 'json':
                ...
                extension = 'txt'
            output_file = Path(vision_dir) / f"vision_analysis.{extension}" 
            ...
            metrics_file = Path(vision_dir) / "vision_metrics.json"
    ...
    def save_results(self):
        # Create a canonical output directory for summary results
        summary_dir = get_canonical_artifact_path("analysis", "summary")
        results_file = Path(summary_dir) / "analysis_summary.json"
...
''',
    ...
    parser.add_argument('--output', '-r', 
                        help='Output directory for results (default: uses canonical artifact path)')
