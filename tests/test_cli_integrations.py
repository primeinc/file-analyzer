...
class TestCLIContracts:
    ...
    def test_fa_artifact_setup_creates_structure(self):
        ...
        for dir_name in required_dirs:
            ...
            assert dir_path.exists(), (
                f"Required directory {dir_name} missing after setup"
            )
    ...
    def test_fa_artifact_path_generates_canonical_paths(self):
        ...
        assert result["stderr"] == "", (
            f"Path generation produced stderr: {result['stderr']}"
        )
        ...
        assert "integration_test" in str(path_obj), (
            "Generated path missing context component"
        )
    ...
    def test_fa_artifact_clean_tmp_empties_directory(self):
        ...
        assert clean_result["success"], (
            f"fa artifact clean-tmp failed: {clean_result['stderr']}"
        )
        assert clean_result["stderr"] == "", (
            f"Clean tmp produced stderr: {clean_result['stderr']}"
        )

        # Verify tmp directory exists but is empty
        tmp_root = Path("artifacts/tmp")
        assert tmp_root.exists(), (
            "Tmp directory removed (should be emptied, not removed)"
        )
        assert len(list(tmp_root.iterdir())) == 0, (
            f"Tmp directory not empty: {list(tmp_root.iterdir())}"
        )
...
class TestCLIErrorHandling:
    ...
    def test_invalid_command_shows_helpful_error(self):
        ...
        assert "invalid-command-xyz" in error_text, (
...
class TestCLIRealWorldUsage:
    ...
    def test_artifact_workflow_end_to_end(self):
        ...
        assert path_result["success"], (
            f"Path generation failed: {path_result['stderr']}"
        )
    ...
    @pytest.mark.skipif(
        not Path("test_data/images/test.jpg").exists(),
    ...
    def test_analyze_with_real_file(self):
        ...
        assert "Traceback" not in result["stderr"], (
            f"Command crashed with traceback: {result['stderr']}"
        )
        ...
        if result["success"]:
            ...
            analysis_outputs = (
                list(artifacts_dir.glob("*")) if artifacts_dir.exists() else []
            )
            assert len(analysis_outputs) > 0, (
                "Successful analysis should create output files"
            )


class TestCLIVersionAndHelp:
    """Test version and help system contracts."""

    def test_version_command_works(self):
        """Version command must show version information."""
        result = run_cmd(["fa", "--version"])

        assert result["success"], f"fa --version failed: {result['stderr']}"
        assert result["stderr"] == "", (
            f"Version command produced stderr: {result['stderr']}"
        )
        assert "File Analyzer" in result["stdout"], (
            "Missing application name in version output"
        )
        assert "Python" in result["stdout"], "Missing Python version in output"

    def test_help_system_comprehensive(self):
        """Help system must be comprehensive and error-free."""
        commands = [
            "analyze",
            "test",
            "validate",
        ...
        for command in commands:
            result = run_cmd(["fa", command, "--help"])
            assert result["success"], f"fa {command} --help failed: {result['stderr']}"
            assert result["stderr"] == "", (
                f"Help for {command} produced stderr: {result['stderr']}"
            )
            assert command in result["stdout"].lower(), (
                f"Help doesn't mention {command}"
            )


class TestCLIPerformance:
    """Test CLI performance contracts."""

    def test_help_commands_are_fast(self):
        """Help commands must execute quickly (under 2 seconds)."""
        import time
        ...
        assert execution_time < 2.0, (
            f"Help command too slow: {execution_time:.2f} seconds"
        )
        assert result["success"], "Help command should succeed"

    def test_version_command_is_fast(self):
        """Version command must execute quickly (under 1 second)."""
        import time
        ...
        assert execution_time < 1.0, (
            f"Version command too slow: {execution_time:.2f} seconds"
        )
