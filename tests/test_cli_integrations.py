...
class TestCLIVersionAndHelp:
    ...
    def test_version_command_works(self):
        ...
        assert result['stderr'] == '', f"Version command produced stderr: {result['stderr']}"
        assert 'File Analyzer' in result['stdout'], "Missing application name in version output"
    ...
    def test_help_system_comprehensive(self):
        ...
        commands = ['analyze', 'test', 'validate', 'artifact', 'install', 'model', 'benchmark']
        
        for command in commands:
            result = run_cmd(['fa', command, '--help'])
            assert result['success'], f"fa {command} --help failed: {result['stderr']}"
            assert result['stderr'] == '', f"Help for {command} produced stderr: {result['stderr']}"
            assert command in result['stdout'].lower(), f"Help doesn't mention {command}"
...
class TestCLIPerformance:
    ...
    def test_help_commands_are_fast(self):
        ...
        assert execution_time < 2.0, f"Help command too slow: {execution_time:.2f} seconds"
    ...
    def test_version_command_is_fast(self):
        ...
        execution_time = end_time - start_time
        assert execution_time < 1.0, f"Version command too slow: {execution_time:.2f} seconds"
