...
class TestBanManualImports:
    ...
    def test_no_manual_command_imports(self):
        ...
        assert not violations, "BANNED MANUAL IMPORTS FOUND:\n" + "\n".join(violations)
    ...
    def test_no_manual_app_registration(self):
        ...
        assert not violations, "BANNED MANUAL APP REGISTRATION FOUND:\n" + "\n".join(
            violations
        )
                ...
                if stripped.startswith("try:"):
                    ...
                    for j in range(i, min(i + 10, len(lines))):
                        ...
                        if "from src.cli." in next_line and "main import" in next_line:
                            violations.append(
                                f"{py_file}:{i}: Manual import in try block"
                            )
                        elif (
                            next_line.startswith("except")
                            or next_line.startswith("def")
                            or next_line.startswith("class")
                        ):
                            break

        assert not violations, "BANNED FALLBACK LOADING FOUND:\n" + "\n".join(
            violations
        )
    ...
    def test_no_dangerous_exception_suppression(self):
        ...
        assert not violations, "DANGEROUS EXCEPTION SUPPRESSION FOUND:\n" + "\n".join(
            violations
        )
    ...
    def test_entry_points_only_architecture(self):
        ...
        assert "entry_points(group=" in content, (
    ...
    def test_clean_command_loading_only(self):
        ...
        for line in lines:
            if "entry_points(group=" in line:
                ...
                assert "from src.cli." not in line, (
                    f"BANNED: Manual import in command loading: {line.strip()}"
                )
                assert "import src.cli." not in line, (
                    f"BANNED: Manual import in command loading: {line.strip()}"
                )
        ...
        if "except" in content and "entry_points" in content:
            ...
            for line in lines:
                if line.strip().startswith("except") and "entry_points" in content:
                    ...
                    assert "from src.cli." not in line, (
                        f"BANNED: Manual import in exception handler: {line.strip()}"
                    )
                    assert "import src.cli." not in line, (
                        f"BANNED: Manual import in exception handler: {line.strip()}"
                    )
